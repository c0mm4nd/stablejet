...
test.describe('数据源和可视化验证', () => {
  ...
  test('验证所有配置的链都有数据', async ({ page }) => {
    ...
    console.log(`\n找到 ${chainCheckboxes.length} 个链配置`);
  ...
  test('验证数据源标识正确显示', async ({ page }) => {
    ...
    const hasKyberSwap = content.toLowerCase().includes('kyberswap') || 
                         content.includes('KS') ||
                         content.includes('🔷');
    const hasOpenOcean = content.toLowerCase().includes('openocean') || 
                         content.includes('OO') ||
                         content.includes('🌊');
    ...
    console.log(`OpenOcean: ${hasOpenOcean ? '✓' : '✗'}`);
    console.log(`Binance: ${hasBinance ? '✓' : '✗'}`);
    
    // 页面上应该至少提到一个数据源（可能在文本中）
    // 即使没有明确标识，页面也应该正常运行
    const hasAnyDataSourceMention = hasKyberSwap || hasOpenOcean || hasBinance;
    console.log(`\n数据源可见性: ${hasAnyDataSourceMention ? '✓ 有标识' : '未显示（可能正常）'}`);
  ...
  test('验证跨链套利机会可视化', async ({ page }) => {
    ...
    console.log(`\n套利可视化: ${hasArbitrageInfo ? '✓ 找到' : '等待数据'}`);
    console.log(`占位符显示: ${hasPlaceholder ? '✓ 正常' : '✗'}`);
  ...
  test('验证金额配置', async ({ page }) => {
    ...
    console.log(`\n总计发现 ${foundAmounts}/${expectedAmounts.length} 个金额配置`);
  ...
  test('验证错误处理显示', async ({ page }) => {
    ...
    console.log(`\n错误处理组件: ${hasErrorHandling ? '有错误显示' : '✓ 无错误 (正常)'}`);
  });
